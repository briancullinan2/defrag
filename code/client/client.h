...
#define cgameUserCmdValue cgameUserCmdValues[igs]
#else
...
#define cmds         cmdWorlds[igs]  // `igvm` because it is based on number of client VMs, not server worlds
...
typedef struct {
	...
	int     selectedWorld; // force the client to load a new VM
	int     selectedClient;
	qboolean      sv_mvWorld;
